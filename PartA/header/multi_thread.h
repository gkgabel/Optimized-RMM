...
struct INPUT{
    ...
    int *matB;
    int *output;
    INPUT(int t_no,int N, int *matA, int *matB, int *output)
    {
        this->t_no=t_no;
        this->N=N;
        this->matA=matA;
...
int n_t=12;
void *singleThread(void * ip)
{
    int t_no=((INPUT *)ip)->t_no,N=((INPUT *)ip)->N,*matA=((INPUT *)ip)->matA,*matB=((INPUT *)ip)->matB, *output=((INPUT *)ip)->output;
    ...
    for(int rowA =((N/n_t+1)+(N/n_t+1)%2)*t_no; rowA <min(N,((N/n_t+1)+(N/n_t+1)%2)*(t_no+1)); rowA +=2) { 
...
void multiThread(int N, int *matA, int *matB, int *output)
{
    ...
    for(int i=0;i<n_t;i++)
    {
        ip[i]=new INPUT(i,N,matA, matB, output);
        int  iret;
        iret = pthread_create( &thread[i], NULL, singleThread, (void *) ip[i]);
    }
    for(int i=0;i<n_t;i++){
